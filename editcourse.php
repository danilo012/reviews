...
<!-- Page -->
	...
	<div class="register-form-hide" style="display: block;">
			<div class="register-form-warp">
				...
				<form class="register-form animate" action="editcourse_ac.php" method="POST">

					<?php
						require_once("connection.php");
						$id=$_REQUEST['id'];
						$query="SELECT * FROM `course_details` WHERE courseid = '$id'";
						$sql=mysqli_query($con,$query);
						if(mysqli_num_rows($sql) > 0)          
						{
							while($row = mysqli_fetch_assoc($sql))
					...
					?>
					<input type="hidden" value="<?php echo($id)?>" placeholder="Course name" name="couid" required="required">
					<input type="text" value="<?php echo($course)?>" placeholder="Course name" name="course" required="required">
		            <input type="text" value="<?php echo($year)?>" placeholder="No. of year" name="year" required="required">
		            <input type="text" value="<?php echo($fees)?>" placeholder="Fees" name="fees" required="required">			
